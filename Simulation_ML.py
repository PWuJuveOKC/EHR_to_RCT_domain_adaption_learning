...
# for 1000 obs only, others 75
my_size = 90

def phi_eta(Frame_X):

    phi = 0.5 * (Frame_X[:, 0] + Frame_X[:, 1] > 0) - \
          (Frame_X[:, 0] + Frame_X[:, 1] <= 0) * (0.5 + 0.5 * (Frame_X[:, 1] <= -0.5)) + (Frame_X[:, 2]**2 - Frame_X[:, 1]**2)
...
def Learning(t):
    ...
    best_model_s3_hi.fit(My_RCT_X3_hi, sim_Q_ran_hi, sim_A_ran_hi, match=np.array([My_RCT_X3_hi.shape[1] - 2,
                        My_RCT_X3_hi.shape[1] - 1]), learn=np.array(range(My_RCT_X3_hi.shape[1] - 2)), bandC=1, size=my_size)
        ...
        for gamma in gammas:
            for kernel in kernels:
                ...
                for train_index, test_index in kf.split(My_RCT_X1):
                    ...
                    est_all = model_all.estimate(X_test, Q_test, A_test, learn=np.array(range(X_test.shape[1] - 1)), normalize=True)
    ...
    best_model_s1 = MatchOLearn_KW(C=PARAM_EST1[0], gamma=PARAM_EST1[1], kernel=PARAM_EST1[2],
                                   metric='mahalanobis', propensity=0.5)
    ...
    print("strategy 3: " , TEST_Potential_ML_s3_hi, TEST_Potential_ML_s3_lo, Counter(TEST_Pred_ML_s3_hi),
           Counter(TEST_Pred_ML_s3_lo), len(TEST_RCT_X3_hi), len(TEST_RCT_X3_lo)),
    print("----------------------------------------------------------------------------------")
...
print(np.median(np.array(results[str(sample_size)])[:,0]),np.mean(np.array(results[str(sample_size)])[:,0]),
      np.max(np.array(results[str(sample_size)])[:,0]), np.std(np.array(results[str(sample_size)])[:,0])),
print(np.median(np.array(results[str(sample_size)])[:,1]),np.mean(np.array(results[str(sample_size)])[:,1]),
      np.max(np.array(results[str(sample_size)])[:,1]), np.std(np.array(results[str(sample_size)])[:,1]))
print(np.median(np.array(results[str(sample_size)])[:,2]),np.mean(np.array(results[str(sample_size)])[:,2]),
      np.max(np.array(results[str(sample_size)])[:,2]), np.std(np.array(results[str(sample_size)])[:,2]))

res = pd.DataFrame(results[str(sample_size)], columns=['s1','s2','s3','s1_ben','s2_ben','s3_ben'])
#res.to_csv('Output_s1/Sim_M_s1_' + whole+ '_' + unknown +'_' + str(ran_size)+ '_'+ str(sim_size[0]) + '_' + strat +'.csv', index=None)
